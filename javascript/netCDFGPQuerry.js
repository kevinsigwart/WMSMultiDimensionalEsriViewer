var gpTask = "";
var inputParamaterName = "in_pnt";
var outputValueField = "water_temp";
var outputTimeField = "time";
var outputZField = "depth";
...
function NetCDFGPQuery(gpTaskString){
    ...
    this.getZfield = netCDFGPQueryGetOutputZField;
...
{
	return outputTimeField;
}

function netCDFGPQueryGetOutputZField()
{
    return outputZField;
}
		...
		load : function(data) {
			...
			dojo.forEach(data.parameters, function(parameter){
				if(parameter.direction == "esriGPParameterDirectionOutput" && parameter.dataType == "GPRecordSet")
				{
					...
					outputTimeField = table.fields[2].name;
                    outputZField = table.fields[1].name;
                    outputValueField = table.fields[3].name;
